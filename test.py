...
cycle_time = 0.5           #機体が1回転する時間
preset_altiude = 100         #着地高度設定
preset_altiude2 = 6       #上昇高度設定
preset_goal_distanc = 5.0  #誘導方法切り替え距離
go_time = 5                #GPS誘導時直進時間
preset_pres = 1000           #!ゴール地点気圧
...
def go2(start, rmax =100, lmax=65 , t = 1.0):
...
def red_search2(camera_count):
    for i in range(1):
        ...
        #画像の2値化
        img_redcorn[(h <255) & (h > 200) & (s > 150)& (v > 150)] = 255 #! (h <255) & (h > 190)の値を変えることによって微妙な赤色の変化に対応できます！
        #画像を出力
...
def log_setup() :
    ...
    f0 = open('logdete.txt', 'w', encoding='UTF-8')
...
def write_log(seqence = "a",gps="0,0",altitude = 0 ,photvolt = 0 ,diredtion = 0 ,my_angle = 0,goal_angle = 0,goal_destance = 0) :
    ...
    f = open('logdete.txt', 'a', encoding='UTF-8')
    f.writelines(dt_now)
    f.writelines(log_list[1:len(log_list)-1:1])
    f.writelines("\n")
    f.close()
    print("write_log")
...
def get_gps() :
    ...
    with open("gps_realtime.txt", "r") as tf:
        gps = tf.read()   
    #gps_realtime = [20.425540,136.081114]　#ダミーデータ
    return gps
...
def main() :
    ...
    if sequence_count == 0 :
        ...
        while True :
            ...
            if photoresistor_voltage > 2 :
                ...
                write_im920flag(1) 
                print("release")
                write_log(sequence_count ,gps = get_gps() ,altitude = get_altitude() ,photvolt = photoresistor_voltage ,
                          diredtion = "release" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                          goal_destance = get_goal_distance(get_gps_realtime(),goal_point)) 
                break
            
            else :
                write_log(sequence_count ,gps = get_gps() ,altitude = get_altitude() ,photvolt = photoresistor_voltage ,
                          diredtion = "no_release" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                          goal_destance = get_goal_distance(get_gps_realtime(),goal_point))
                time.sleep(0.1)  
    
    #第二空中シーケンス(光センサーの使用が不可の場合に使用)
    if sequence_count == 0.5 :
        altitude_flag = 0 
        while True :
            if get_altitude() > preset_altiude2 :
                print("rise")
                
                if altitude_flag < 3 :
                    altitude_flag = altitude_flag + 1
                    write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
    ...
    if sequence_count == 1 :
        f = 1
        while True :
            if get_altitude() < preset_altiude :
                ...
                write_sequence_count(sequence_count)
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
                          diredtion = "ignition_1" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                          goal_destance = get_goal_distance(get_gps_realtime(),goal_point))
                break
                ...
                write_sequence_count(sequence_count)
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0,
                          diredtion = "ignition_2" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                          goal_destance = get_goal_distance(get_gps_realtime(),goal_point))
                break
            
            else :
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
                          diredtion = "descent" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                          goal_destance = get_goal_distance(get_gps_realtime(),goal_point))
                time.sleep(0.06)
                f = f + 0.1
                print(f)
    ...
    if sequence_count == 2 :  
        ...
        write_sequence_count(sequence_count)
        write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
                  diredtion = "escape" ,my_angle = 0 ,goal_angle = get_goal_angle(get_gps_realtime(),goal_point) ,
                  goal_destance = get_goal_distance(get_gps_realtime(),goal_point))
        print("脱出")
    ...
    if sequence_count == 3 : 
        ...
        while True :
            ...
            if past_point == my_point :
                stack_count = 0 
                #機体の方位を取得
                #my_angle = get_myangle2(past_point, my_point)
                ...
                if goal_distance < 5.00 :
                    write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
                              diredtion = 0 ,my_angle = get_myangle2(past_point, my_point),goal_angle = get_goal_angle(my_point,goal_point),
                              goal_destance = get_goal_distance(my_point,goal_point))
                    sequence_count = 4
                    write_sequence_count(sequence_count)
                    break
                ...
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 ,
                          diredtion = "stack" ,my_angle = get_myangle2(past_point, my_point),goal_angle = get_goal_angle(my_point,goal_point),
                          goal_destance = get_goal_distance(my_point,goal_point))
    ...
    if sequence_count == 4 :
        ...
        threshold =50              #カメラ誘導時赤色認識した時に直進するかしないかどうか（0~320） 
        while True :
            ...
            if out_re[0] == 0 and out_re[1] == 0 and f1 == 0 :
                ...
                stop()
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude() ,photvolt = 0 
                              ,diredtion = "tansakumo-do" ,my_angle = 0 ,goal_angle = 0 ,goal_destance = 0)
